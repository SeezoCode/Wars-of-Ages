...
var troopArr = [
    ...
    {
        name: 'Boomer Troop', health: 1, damage: 49, baseDamage: 30, attackSpeed: 17, castingTime: 1.6, price: 60, color: 'red', speed: 1, span: 20, range: 21, researchPrice: 550
    },
    {
...
var Game = /** @class */ (function () {
    ...
    function Game(player1, player2, visualize, playerUnits1, playerUnits2) {
        ...
        this.players[0].map(this.players[1], visualize, this.playerOneUnits, playerUnits1, this.playerTwoUnits, this.playerTwoBase, this.playerOneBase, this);
        this.players[1].map(this.players[0], visualize, this.playerTwoUnits, playerUnits2, this.playerOneUnits, this.playerOneBase, this.playerTwoBase, this);
        this.animation();
    }
...
var Player = /** @class */ (function () {
    ...
    Player.prototype.map = function (enemy, visualize, playerUnits, startingPlayerUnits, enemyUnits, enemyBase, playerBase, game) {
        var _this = this;
        this.enemy = enemy;
        this.visualize = visualize;
...
var SimulatingBot = /** @class */ (function (_super) {
    ...
    SimulatingBot.prototype.afterMoveArmy = function () {
        ...
        if (this.cooldown <= 0) {
            // console.log(enc)
            if (enc >= .50) {
                var bestTroops = this.bruteforceWorker(this.parseUnits(this.playerUnits), this.parseUnits(this.enemyUnits));
                // console.log(bestTroops)
                if (enc === 10)
                    console.log(bestTroops);
                if (bestTroops.length && this.playerUnits.length < 7)
        ...
        this.cooldown--;
        document.getElementById("enc" + this.side).innerText = String(Math.round(enc * 100) / 100);
    };
    SimulatingBot.prototype.pushPull = function () {
    ...
    // return bestTroops
    // }
    SimulatingBot.prototype.bruteforceWorker = function (playerTroops, enemyTroops) {
        var worker = new Worker('index.js');
        worker.postMessage([playerTroops, enemyTroops, this.unlockedUnits, this.side]);
    ...
    SimulatingBot.prototype.damageCalc = function (stats) {
        if (stats.playerSpending === 0 || !this.isEnoughMoney(stats.playerSpending))
            return -1;
            ...
            return (stats.playerDamage) / (stats.playerSpending / this.encouragement());
        else {
    ...
    SimulatingBot.prototype.encouragement = function () {
        ...
        if (stats.playerUnitsLength && stats.enemyUnitsDamage) {
            return Math.sqrt((stats.enemyUnitsLength / stats.playerUnitsLength) / (stats.playerUnitsDamage / stats.enemyUnitsDamage));
        }
        else if (!stats.enemyUnitsLength)
            return 0;
        else {
...
onmessage = function (e) {
    ...
    e.data[2].forEach(function (e) { return e ? numberOfUnlockedUnits++ : 0; });
    console.log(numberOfUnlockedUnits)
